...
char** sh_split_line(char* line)
{
	...
	if(!token)
	{
		fprintf(stderr,"amey-sh: allocation error\n");
		exit(EXIT_FAILURE);
	}
...
int sh_wikipedia(char** args){
	if(args[1]==NULL){
		char line1[]="xdg-open https://www.wikipedia.org/";
		char **args1=sh_split_line(line1);
		return sh_launch(args1);
	}
	if(args[2]!=NULL){
	...
	char command[1000]="xdg-open https://en.wikipedia.org/wiki/Special:Search?search=&go=Go";
	char* search_query=args[1];
	strcat(command,search_query);
	char **args2=sh_split_line(command);
	return sh_launch(args2);
